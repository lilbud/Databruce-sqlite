...
def get_venues():
	...
	url = requests.get(f"{main_url}system:sitemap", timeout=5).text
	soup = bs4(url, "lxml")
	venues = []
...
def get_show_info(url):
	...
	if r.status_code == 200:
		...
		v = cur.execute(f"""SELECT venue_url FROM VENUES WHERE venue_url LIKE '{venue.get('href')}'""").fetchone()
		...
		for n in nav:
			...
			if n.text == "Setlist":
				get_setlist_by_url(soup.find(id=f"wiki-tab-0-{nav.index(n)}"), url, date[0])
	else:
...
def get_tours():
	...
	tour = []
	df = pd.read_html(f"{main_url}stats:tour-statistics", extract_links="all", skiprows=(0,1))
...
def get_tour_events(url, name):
	...
	r = requests.get(f"{main_url}{url.strip('/')}", timeout=10).text
	soup = bs4(r, "lxml")
