...
albums = {
    "Greetings From Asbury Park, N.J.": [1973, "studio"],
...
def run_time(start_time):
    ...
    print(f"Runtime: {str(end_time - start_time).split('.', maxsplit=1)[0]}")
...
def show_name_split(show_name, url):
    ...
    if state in states:
        ...
        country = vn[-1].strip()
        if re.findall(r"\(.*\)", country):
            show_type = re.findall(r"\(.*\)", country)
